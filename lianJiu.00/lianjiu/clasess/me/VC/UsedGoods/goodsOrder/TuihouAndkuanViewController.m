...
@interface TuihouAndkuanViewController ()<UITextViewDelegate,UITextFieldDelegate>
...
@implementation TuihouAndkuanViewController
{
...
- (void)viewDidLoad {
    ...
    titleL.text = _model.orItemsNamePreview?_model.orItemsNamePreview:@"";//@"苹果了房间啊";
   ...
   // contentTV.text = @"fasdkf;aksd;f'kas;klads";
...
//#pragma mark-货物状态:
//    UILabel *hTitleL = [[UILabel alloc] initWithFrame:CGRectMake(10, CutLine.frame.size.height+CutLine.frame.origin.y+8, 70, 21)];
//    hTitleL.text = @"货物状态:";
//    // fTitleL.backgroundColor = [UIColor blueColor];
...
//    CGFloat qbtnW2 = 85;
//    //(BOUND_WIDTH-leftgap-qbtngap*(qarr.count-1))/qarr.count;
//    UIView *qbtnView2 = [[UIView alloc] initWithFrame:CGRectMake(leftgap2, CutLine.frame.size.height+CutLine.frame.origin.y+4,BOUND_WIDTH-leftgap2 , qbtnH2)];
...
//    [contenV addSubview:CutLine2];
    ...
    UILabel *tTitleL = [[UILabel alloc] initWithFrame:CGRectMake(10, kxCutLine.frame.size.height+kxCutLine.frame.origin.y+8, 70, 21)];
    tTitleL.text = @"退货原因:";
    tTitleL.font =PFR15Font;
    [contenV addSubview:tTitleL];
    descriptiontv = [[UITextView alloc] initWithFrame:CGRectMake(80, kxCutLine.frame.size.height+kxCutLine.frame.origin.y+8, BOUND_WIDTH-80-10, 120)];
...
//    }
    ...
    if (descriptiontv.text.length==0) {
        [MBProgressHUD showNotPhotoError:@"请填写退货原因" toView:self.view];
        return;
    }

    if (descriptiontf.text.length==0) {
        [MBProgressHUD showNotPhotoError:@"请填写退货金额" toView:self.view];
        return;
    }
    if ([descriptiontf.text floatValue]==0) {
        [MBProgressHUD showNotPhotoError:@"退货金额最少为0.01" toView:self.view];
        return;
    }
    ...
    [networking GAFNPOSTMutableArrfile:LOADIMGLJ withparameters:nil dataName:@"file" dataSource:dataArr success:^(NSMutableDictionary *dic) {
        ...
        NSDictionary *para = @{@"orExcellentId":_model.ordersId,@"orExceRefundCause":descriptiontv.text,@"orExceRefundMoney":descriptiontf.text,@"orExceRefundExpresspic":mutStr};
        ...
        [networking AFNPOST:ORDERSREFUNDGOODJP withparameters:para success:^(NSMutableDictionary *dic) {
        ...
        //    [MBProgressHUD showNotPhotoError:@"退款成功" toView:self.view];
            ...
            //提示框
            UIAlertController *clear = [UIAlertController alertControllerWithTitle:@"提示" message:@"退货退款单已提交成功，请耐心等待。我们将尽快为您受理！" preferredStyle:UIAlertControllerStyleAlert];
...
//{
//    
//    return CGSizeMake((BOUND_WIDTH-95)/3, (BOUND_WIDTH-95)/3);
//}
