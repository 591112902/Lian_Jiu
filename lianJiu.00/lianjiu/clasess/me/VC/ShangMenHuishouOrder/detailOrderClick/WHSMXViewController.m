...
#import "WHSMXViewController.h"
#import "WHSMXTableViewCell.h"
@interface WHSMXViewController ()<UITableViewDataSource,UITableViewDelegate>

@end

@implementation WHSMXViewController
{
...
- (void)viewDidLoad {
    ...
    reuseIdentifier = @"WHSMXTableViewCell";
...
//    }
   ...
   // [NSString stringWithFormat:@"%@/%@",GETORDERS,_orFacefaceId];
    ...
    [networking AFNRequestNotHUD:[NSString stringWithFormat:@"%@/%@",SELECTORDERDETAILS,_orFacefaceId] withparameters:nil success:^(NSMutableDictionary *dic) {
        
        NSLog(@"GETORDERS:%@%@",dic,[NSString stringWithFormat:@"%@/%@",@"",_orFacefaceId]);
...
-(void)zjmxBtnClick:(UIButton*)btn{
    ...
    //上门-家电
    if ([model.orItemsStatus isEqualToNumber:@0]||[model.orItemsStatus isEqualToNumber:@1]||[model.orItemsStatus isEqualToNumber:@2]||[model.orItemsStatus isEqualToNumber:@4]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row%2==0) {
        ...
        WHSMXTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
            MingXiDanModel *moel = _dataSour[indexPath.row/2];
            [cell fillCellWithModel:moel];
        
        
        cell.zjmxBtn.tag = indexPath.row/2+460000;
