...
- (void)setupUI
{
    ...
    _textField.text = [NSString stringWithFormat:@"%zd",_minValue];
...
- (void)increase
{
    ...
    if (number <= _maxValue) {
        ...
        _textField.text = [NSString stringWithFormat:@"%zd", number];
        
        [self buttonClickCallBackWithIncreaseStatus:YES];
    } else {
        if (_shakeAnimation) { [self shakeAnimationMethod]; } NSLog(@"已超过最大数量%zd",_maxValue);
        [MBProgressHUD showNotPhotoError:@"已超过最大数量" toView:[UIApplication sharedApplication].keyWindow];
    }
}
...
- (void)decrease
{
    ...
    if (number >= _minValue) {
        ...
        if (_decreaseHide && number<_minValue) {
            ...
            _textField.text = [NSString stringWithFormat:@"%zd",_minValue-1];
        ...
        if (_shakeAnimation) { [self shakeAnimationMethod]; } NSLog(@"数量不能小于%zd",_minValue);
        [MBProgressHUD showNotPhotoError:@"不能再减了" toView:[UIApplication sharedApplication].keyWindow];
    }
}
...
- (void)checkTextFieldNumberWithUpdate
{
    ...
    if ([_textField.text pp_isNotBlank] == NO || _textField.text.integerValue < _minValue) {
        _textField.text = _decreaseHide ? [NSString stringWithFormat:@"%zd",minValueString.integerValue-1]:minValueString;
    }
...
{
    _minValue = minValue;
    _textField.text = [NSString stringWithFormat:@"%zd",minValue];
}
...
- (void)setCurrentNumber:(NSInteger)currentNumber
{
    ...
    _textField.text = [NSString stringWithFormat:@"%zd",currentNumber];
    [self checkTextFieldNumberWithUpdate];
}
