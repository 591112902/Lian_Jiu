...
@implementation SetUpListModel
+(instancetype)ModelWith:(NSDictionary *)dic
{
    SetUpListModel *model = [[self alloc] init];
    
    
    model.bulkId = dic[@"bulkId"];
    model.bulkName = dic[@"bulkName"];
    model.bulkVolume = dic[@"bulkVolume"];
    model.bulkNum = dic[@"bulkNum"];
    model.priceSingle = dic[@"priceSingle"];
    model.categoryId = dic[@"categoryId"];
    model.priceUnit = dic[@"priceUnit"];
