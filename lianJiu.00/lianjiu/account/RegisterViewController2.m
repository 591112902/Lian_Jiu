...
- (void)viewDidLoad {
    ...
    [self.quHaoBtn setTitle:CustomLocalizedString(@"选择区号", nil) forState:UIControlStateNormal];
    self.quHaoBtn.titleLabel.adjustsFontSizeToFitWidth = YES;
    ...
    //self.accoutTF.placeholder = @"shouji 手机手机";
...
- (IBAction)ToRegister:(id)sender {
    ...
    if (
        ...
        NSDictionary *parameters = @{@"area":_quhaozhi,@"name":self.accoutTF.text,@"code":self.codeTF.text,@"pwd":self.passwordAgainTF.text,@"referrer_vip":self.RecommendedCodeTF.text};
    ...
    //[MBProgressHUD showSuccess:@"程序员哥哥正在开发中...." toView:self.view.window];
   ...
   // [MBProgressHUD showSuccess:@"程序员哥哥正在开发中...." toView:self.view.window];
    if (!_quhaozhi) {
        ...
        if (self.accoutTF.text.length!=0) {
            NSDictionary *parameters = @{@"area":_quhaozhi,@"name":self.accoutTF.text,@"type":@"1"};
            [networking AFNPOST:@"" withparameters:parameters success:^(NSMutableDictionary *dic) {
                code = dic[@"response"];
                NSLog(@"获取验证码:%@",dic);
            ...
            [MBProgressHUD showNotPhotoError:CustomLocalizedString(@"手机号码不能为空", nil) toView:[UIApplication sharedApplication].keyWindow];
        }
    }
}
-(void)timeFireMethod{
