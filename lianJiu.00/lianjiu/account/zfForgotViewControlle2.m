...
#import "validate.h"
@interface zfForgotViewController2 ()
...
@implementation zfForgotViewController2
{
    NSString *accout;
    NSString *code;
...
- (void)viewDidLoad {
    ...
    [self.quhaoForgotBtn setTitle:CustomLocalizedString(@"选择区号", nil) forState:UIControlStateNormal];
    self.quhaoForgotBtn.titleLabel.adjustsFontSizeToFitWidth = YES;
...
- (IBAction)getCode:(id)sender {
    //NSLog(@"_quhaozhi_quhaozhi:%@",_quhaozhi);
    if (!_quhaozhi) {
        [MBProgressHUD showSuccess:CustomLocalizedString(@"请选择区号", nil) toView:[UIApplication sharedApplication].keyWindow];
        return;
    }
    ...
    if (self.accoutTF.text.length!=0) {
      ...
      NSString *vip =  [def objectForKey:@"vip"];
        ...
        NSDictionary *parameters = @{@"vip":vip,@"area":_quhaozhi?_quhaozhi:@"+86",@"name":self.accoutTF.text,@"type":@"2"};
        [networking AFNPOST:@"" withparameters:parameters success:^(NSMutableDictionary *dic) {
            code = dic[@"response"];
            HZLog(@"%@",dic);
            [MBProgressHUD showSuccess:CustomLocalizedString(@"验证码已发送成功", nil) toView:[UIApplication sharedApplication].keyWindow];
        ...
        [MBProgressHUD showSuccess:CustomLocalizedString(@"请输入手机号", nil) toView:[UIApplication sharedApplication].keyWindow];
...
- (IBAction)findPassword:(id)sender {
    ...
    if (
        [validate CheckInputALLPhone:CustomLocalizedString(@"手机号码", nil) withText:self.accoutTF.text]&&[validate CheckInputAccount:CustomLocalizedString(@"支付密码", nil) withText:self.passwordTF.text withminNum:6 withMaxNum:14000]&&
        ...
        NSUserDefaults *def = [NSUserDefaults standardUserDefaults];
            ...
            NSDictionary *PayParmeters=@{@"vip":[def objectForKey:@"vip"],@"area":_quhaozhi,@"newpayment":self.passwordTF.text,@"code":self.codeTF.text,@"name":self.accoutTF.text};
