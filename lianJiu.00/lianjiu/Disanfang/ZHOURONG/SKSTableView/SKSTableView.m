...
- (NSMutableDictionary *)expandableCells
{
    if (!_expandableCells)
    {
        ...
        // ++++++++++++＋＋＋
        [self setExpanded:YES forCellAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        UITableViewCell *cell = [_SKSTableViewDelegate tableView:(SKSTableView *)tableView cellForSubRowAtIndexPath:correspondingIndexPath realIndexPath:indexPath];
            ...
            if (_indexPath)
            {
                ...
                else
                {
                    ...
                    if ((indexPath.row+1) < rows.count) {
                        [self tableView:tableView didSelectRowAtIndexPath:[NSIndexPath indexPathForRow:(indexPath.row+1) inSection:indexPath.section]];
                    } else if ([[self.expandableCells allKeys] containsObject:@(indexPath.section+1)]) {
                        if ((indexPath.section+1) < ([self.expandableCells allKeys].count < 3?[self.expandableCells allKeys].count:([self.expandableCells allKeys].count-1))) {
                            [self tableView:tableView didSelectRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:(indexPath.section+1)]];
                        }
                    }
                }
...
-(void)collapseCellForIndexPath:(NSIndexPath *)indexPath{
//    NSIndexPath *correspondingIndexPath = [self correspondingIndexPathForRowAtIndexPath:indexPath];
//    [self setExpanded:false forCellAtIndexPath:correspondingIndexPath];
//    SKSTableViewCell *cell = (SKSTableViewCell *)[self cellForRowAtIndexPath:indexPath];
//    cell.expanded = NO;
//    [cell accessoryViewAnimation];
//    [self deleteRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:indexPath.row+1 inSection:indexPath.section]] withRowAnimation:UITableViewRowAnimationTop];
